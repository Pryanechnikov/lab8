...
<div class="btn-group d-flex justify-content-center mb-4" role="group" aria-label="Basic example">
    <a href="?filter=На рассмотрении" class="btn btn-secondary">На рассмотрении</a>
    <a href="?filter=В работе" class="btn btn-secondary">В работе</a>
    <a href="?filter=Готово" class="btn btn-secondary">Готово</a>
    <a href="?filter=Все" class="btn btn-secondary">Все</a>
</div>
    ...
    <div class="col">
        ...
        <div class="d-flex flex-wrap">
            <? foreach ($arResult["ITEMS"] as $arItem): ?>
                <?
                if ($arItem['PROPERTIES']['STATUS']['~VALUE'] === $_GET['filter'] || $_GET['filter'] === 'Все' || $_GET['filter'] === null){
                $this->AddEditAction(
                    $arItem['ID'],
                    $arItem['EDIT_LINK'],
                ...
                ?>
                <div class="news-list-item mb-3 ml-2 mr-2" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
                    <div class="card">
                        <? if ($arParams["DISPLAY_PICTURE"] != "N"): ?>
                            ...
                            if ($arItem["SLIDER"] && count($arItem["SLIDER"]) > 1)
                            {
                            ?>
                            ...
                            <? foreach ($arItem["FIELDS"] as $code => $value): ?>
                                ...
                                <? elseif (
                                    ...
                                    $value = CIBlockFormatProperties::DateFormat($arParams["ACTIVE_DATE_FORMAT"],
                                        MakeTimeStamp($value, CSite::GetDateFormat()));
                                    ?>
                                    <div class="news-list-view news-list-post-params">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <? if (!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])): ?>
                                    <div class="news-list-more">
                                        <a class="btn btn-primary btn-sm" href="<? echo $arItem["DETAIL_PAGE_URL"] ?>">Подробнее</a>
                                    </div>
                                <? endif; ?>
